(function(factory) {
    ...
    var factory = function($) {
		...
		(function(win, doc, undefined) {
			...
			//============================业务代码===========================
			var config = {
				clientUrl: "http://m1.playsnail.com/linkComet.html"
			};
			var crossDomain = getCrossDomain(config);
			if (crossDomain.isMain()) {
				function _boxRefresh(data) {
					function getNum (n) {
						return (n > 0 ? '(' + n + ')' : '');
					}
					var data = eval("(" + data + ")");
					...
					if(!data.code) {
						...
						var total = 0;
						for(var i in inboxNum) {
							$('.inbox' + i + 'Show').each(function() {
								if(this.innerHTML == "") {
									var _nowNum = 0;
								} else {
									var _nowNum = this.innerHTML.replace('(', '').replace(')', '');
								}
								this.innerHTML = getNum(data[i] + parseInt(_nowNum));
								if($(this).parents('.banner_menu_xl').length >= 1) {
									total = total + data[i] + parseInt(_nowNum);
								}
							})
						};
						
						if($(".inboxtotalShow").length > 0) {
							$(".inboxtotalShow").html(getNum(total));
						}
						
						if($("#info").length > 0) {
							
							if($("#info").attr('class').indexOf('game_embedded_menu_b_Se') > -1) {
								$("#info").addClass('game_embedded_menu_b_msg_Se');
							} else {
								$("#info").addClass('game_embedded_menu_b_msg');
							}
						...
						if($("#myName").length > 0) {
							...
							var tt = 0;
							for(var i in data) {
								tt = tt + parseInt(data[i]);
							}
							if($("#myName").html().indexOf('(') > -1) {
								var now = $("#myName").text().substr($("#myName").text().indexOf('(')).replace(/[^0-9]/ig, "");
								tt = tt + parseInt(now);
								$("#myName").html($("#myName").text().substr(0, $("#myName").text().indexOf('(')) + "<a href='/minips/9yin/person/inbox/index.html'>" + getNum(tt) + "</a>");
							} else {
								$("#myName").html($("#myName").text() + "<a href='/minips/9yin/person/inbox/index.html'>" + getNum(tt) + "</a>");
							}
						}
						
						window.document.title = getNum(total) + window.document.title.replace(/\(\d+\)/,'');
						if(data.clanWeibo > 0) {
							if($("#UpdateCountDiv").length > 0) {
								var _nowNum = $("#UpdateCountDiv").html().replace(/[^0-9]/ig, "");
								$("#UpdateCountDiv").show().html((data.clanWeibo + parseInt(_nowNum)) + 'New Posts');
							}
						}
						if(data.personalWeibo > 0) {
							if($("#UpdateCountDiv").length > 0) {
								var _nowNum = $("#UpdateCountDiv").html().replace(/[^0-9]/ig, "");
								$("#UpdateCountDiv").show().html((data.personalWeibo + parseInt(_nowNum)) + 'New Posts');
							}
						}
					}
				};
				crossDomain.mainCommand.add('boxRefresh', _boxRefresh);
				setTimeout(function () {
					$('body').bind('mousemove', function(event) {
						if(!!$.cookie('nUserId')) {
							crossDomain.message.send('connect');
						}
					});
				},5000);
			};
				...
				var comet = (function() {
					...
					var util = {
						getCookie: function (key) {
							return RegExp("(?:; )?" + key + "=([^;]*);?").test(doc.cookie) ? decodeURIComponent(RegExp.$1) : null
						},
						ajax: function (opts) {
							var xhr = getXHR ();
							var timeId = 0;
							xhr.onreadystatechange = function (date) {
								if(xhr.readyState==4){
									clearTimeout(timeId);
									if(xhr.status==200|| status == 304){
										opts.success.call(xhr, xhr.responseText);
									}
									opts.complete.call();
								}
							};
							xhr.open('GET',opts.url+'?'+opts.data+'&_'+new Date().valueOf(),true);
							xhr.send();
					...
					return {
						...
						ajaxLoop : function() {
							...
							var _this = this;
							util.ajax({
								url : 'http://m1.playsnail.com/platform/message/subscribe',
								data : 'userId=' + util.getCookie('nUserId'),
								timeout : 70000,
						...
						handleResponse : function(data) {
							crossDomain.message.send('boxRefresh', data);
						}
					}
				})();
				crossDomain.clientCommand.add('connect', function(callback) {
					callback('');
					comet.connect();
				});
			}
